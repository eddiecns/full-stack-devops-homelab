...
GitLab and Kubernetes VMs are provisioned using the `bpg/proxmox` Terraform provider, cloning from a cloud-init-ready Rocky Linux 9 template. Each VM has its own isolated project folder and state file.

**Ansible as the CD intermediary**
The DevOps LXC acts as both the Ansible control node for infrastructure config and the deployment broker in the CD pipeline. Jenkins SCPs image tarballs here; Ansible then distributes them to Kubernetes nodes and triggers the rollout.

**No container registry**
Images are built on Jenkins, saved as `.tar.gz`, and distributed via `scp` + `ctr import`. This avoids registry infrastructure overhead while keeping the pipeline functional in an air-gapped home lab environment.

**Init containers for dependency ordering**
The Kubernetes deployment uses two init containers: one to wait for MySQL readiness and run migrations, one to run `collectstatic`. This ensures correct startup order without external orchestration.

**Git SHA image tagging**
Every Docker image is tagged with the git commit SHA, enabling precise traceability between a running pod and the exact code revision it was built from.
...
| Tool          | Purpose                              | URL                              |
|---------------|--------------------------------------|----------------------------------|
| Prometheus    | Scrapes kubelet metrics (port 10250) | http://192.168.1.10:9090         |
| Grafana       | Kubernetes cluster dashboard (ID 315)| http://192.168.1.10:3000         |
